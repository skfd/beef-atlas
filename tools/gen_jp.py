...
cut("neck", "Neck", "ネック", "nekku",
    "The neck, between the poll at x 0.80 and the point of shoulder at x 0.72, high on the topline.",
    [0.72, 0.82], [0.74, 0.94], True,
    "One of the thirteen official 部分肉 and among the hardest-working muscles on the animal, so it is coarse-grained, almost free of sashi and firm, but very rich in collagen and beefy flavour. It is almost never sold as a steak cut — it goes to mince, stock and long-simmered dishes.",
    ["nikomi (stew)", "curry", "hamburg", "soup stock"])

cut("kata", "Chuck / shoulder clod (arm)", "カタ", "kata",
    "The foreleg from the knee at z 0.28 up to the elbow and shoulder joint at z 0.54, x 0.66–0.74; the front shank sits inside this box.",
    [0.66, 0.74], [0.28, 0.54], False,
    "The 部分肉 called カタ (also ウデ, arm) is a bundle of well-exercised muscles — lean, slightly tough, modestly marbled but loaded with gelatin and umami — and Japanese butchery counts the front shank within it rather than with スネ. Sold thin-sliced for sukiyaki and as budget yakiniku, or cubed for stews; the rare cuts ミスジ, トウガラシ and クリミ are carved out of it.",
    ["nikomi (stew)", "sukiyaki", "yakiniku", "soup stock"])

cut("misuji", "Top blade / flat iron", "ミスジ", "misuji",
    "On the outer face of the shoulder blade, above the elbow and behind the point of shoulder, x 0.66–0.74, z 0.54–0.66.",
...
cut("tougarashi", "Chuck tender / shoulder petite tender", "トウガラシ", "tougarashi",
    "On the front face of the shoulder blade at the point of shoulder, x 0.70–0.77, just below the neck line at z 0.66–0.74.",
    [0.70, 0.77], [0.66, 0.74], False,
    "Named for its chili-pepper shape, this cone of lean muscle sits against the scapula inside the カタ primal; it has very little sashi and a clean red-meat flavour with a slight chew. Prized as a lean yakiniku cut and for roast beef, and one of the 希少部位 sold mainly by specialist shops.",
...
cut("sirloin", "Strip loin / sirloin", "サーロイン", "saroin",
    "The back from the last rib at x 0.46 rearward to the hip at x 0.30, along the topline z 0.76–0.92.",
    [0.30, 0.46], [0.76, 0.92], True,
    "Despite the borrowed name, Japanese サーロイン corresponds to the Anglo strip loin — the loin eye over the lumbar vertebrae, the fillet beneath it counted separately, and the sirloin butt further back belonging to ランイチ. The muscle does almost no work, so it takes the finest, sweetest, most even sashi on the carcass; it is the one cut Japan does treat as a thick steak, though it is shaved for sukiyaki just as often.",
...
cut("katabara", "Brisket / chuck rib (front plate)", "カタバラ", "katabara",
    "The front of the belly and chest, from the fifth-rib line at x 0.56 forward to the brisket behind the elbow at x 0.66, z 0.46–0.66.",
    [0.56, 0.66], [0.46, 0.66], True,
...
cut("karubi", "Short rib (kalbi)", "カルビ", "karubi",
    "The rib-cage section of the belly, x 0.44–0.56 between the last rib and the mid ribs, z 0.46–0.58.",
    [0.44, 0.56], [0.46, 0.58], True,
...
cut("tomobara", "Short plate / flank", "トモバラ", "tomobara",
    "The rear belly hanging below the barrel, x 0.32–0.44, from the belly line z 0.56 down to 0.46.",
    [0.32, 0.44], [0.46, 0.56], True,
...
cut("ranpu", "Rump", "ランプ", "ranpu",
    "The top of the hindquarter above the hip joint, behind the sirloin line, x 0.16–0.26, z 0.66–0.90.",
    [0.16, 0.26], [0.66, 0.90], False,
    "The upper part of the ランイチ 部分肉 (ランプ plus イチボ), sitting where the loin runs out over the hip — this is the sirloin butt of Anglo butchery. Fine-grained, tender lean meat with a light even sashi and a concentrated beef flavour for a hindquarter cut; served as steak, as thin yakiniku slices and in sukiyaki.",
...
cut("ichibo", "Rump cap / picanha", "イチボ", "ichibo",
    ...
    "The lower, rearmost muscle of the ランイチ 部分肉, named after the H-shaped hip bone (H-bone) it caps; lean but carrying a fat cap and fine marbling, tender at the tail end and firmer toward the leg. Grilled as thick yakiniku slices or roasted whole.",
    ["yakiniku", "steak", "roast beef"])

cut("uchimomo", "Inside round / topside", "ウチモモ", "uchimomo",
    "The inner face of the hind leg between the stifle at z 0.46 and the rump at z 0.62, x 0.12–0.24.",
    [0.12, 0.24], [0.46, 0.62], False,
...
cut("sotomomo", "Outside round / silverside", "ソトモモ", "sotomomo",
    "The outer and rearmost face of the hind leg, x 0.00–0.12, from the stifle at z 0.46 up to z 0.62.",
    [0.00, 0.12], [0.46, 0.62], False,
...
cut("shintama", "Knuckle / thick flank", "シンタマ", "shintama",
    "The ball of muscle on the front of the hind leg around the femur, x 0.24–0.32, z 0.52–0.64.",
    [0.24, 0.32], [0.52, 0.64], False,
...
cut("shinshin", "Knuckle centre / centre of the ball tip", "シンシン", "shinshin",
    "The core of the knuckle, low on the front of the hind leg just above the stifle, x 0.24–0.32, z 0.46–0.52.",
    [0.24, 0.32], [0.46, 0.52], False,
...
cut("sune", "Shank / shin", "スネ", "sune",
    "The hind leg between the hock at z 0.28 and the stifle at z 0.46, x 0.03–0.18; the foreleg equivalent is counted inside カタ.",
    [0.03, 0.18], [0.28, 0.46], False,
...
assert not bad, "overlapping boxes"
...
print("z range:", min(c["z"][0] for c in C), max(c["z"][1] for c in C))
